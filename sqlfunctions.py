...
def AddNewUser(username, password, admin, staysignedin, primary_admin=False):
    if admin:
        if primary_admin:
            if staysignedin == 1:
                mycursor.execute(
                    "INSERT INTO LOGINDATA(username, password, usertype, signedin, primary_admin)"
                    " VALUES(%s, %s, %s, %s, %s)",
...
def UpdateActivity(username, logintime, date, turnover=0):
    ...
    mycursor.execute(
        f"INSERT INTO activitylog(username, date, login_time, logout_time, turnover)"
        f" VALUES(%s, %s, %s, %s, {turnover})",
